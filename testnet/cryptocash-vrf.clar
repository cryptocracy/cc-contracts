...
(define-read-only (get-random-uint-at-block (blockHeight uint))
  (let (
    (vrf-lower-uint-opt
      (match (get-burn-block-info? header-hash blockHeight)
        header-hash (some (buff-to-uint-le (lower-16-le header-hash)))
        none))
  )
  (print {event:"get-random-uint-at-block", blockHeight: blockHeight, hash: vrf-lower-uint-opt})
  vrf-lower-uint-opt)
)
